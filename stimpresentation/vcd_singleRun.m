...
if ~isfield(params, 'stim') || isempty(params.stim)
    if params.loadparams
        ...
        if isempty(d)
            ...
            warning('[%s]: Can''t find stim params file for %s, will reload params without overwriting params',mfilename,params.disp.name);
            params.stim  = vcd_getStimParams('disp_name', params.disp.name, ...
    ...
    if ~isempty(params.timetable_file)
        ...
        if ~isempty(d)
            ...
            if ~isempty(d)
                ...
                fprintf('[%s]: **** WARNING **** NO GENERAL CONDITION_MASTER FILE FOUND!! Will create new general condition_master file.\n',mfilename);
                [~, condition_master] = ...
                    vcd_createConditions(params, ...
                    'load_params',  false, ...
                    'store_params', true, ...
                    'env_type', params.env_type);
                ...
                [~,~,time_table_master, all_run_frames] = ...
                    ...
                    'load_params',  false, ...
                    'store_params', true, ...
                    'condition_master',condition_master,...
                    'env_type',params.env_type, ...
                    'saveDir',params.timetablefiledir, ...
                    'subj_id',sprintf('vcd_subj%03d',params.subj_nr));
            end
