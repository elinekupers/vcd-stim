...
% CONSTRAINTS
max_diff_leftright_cueing = 0; % equal left/right cueing for classic stim
...
for ses = session_nrs
    ...
    stypes = ~isnan(session_types(ses,:));
    for st = session_types(ses,stypes)
        if ~isnan(session_types(ses,st))
            ...
            if session_flag
                ...
                % Collapse scc and ltm classic stim blocks
                for ii = [3,6]
                    collapsed_blocks = sum(stimtask_counter(:,ii));
                    stimtask_counter(1,ii) = collapsed_blocks;
                    stimtask_counter(2:end,ii) = 0;
                end
                    ...
                    for tc = 2:size(stimtask_counter,2) % skip fixation
                        ...
                        if ismember(tc,[3,6])
                            if sc == 1 % scc or ltm
                                ...
                                nr_blocks = sum(stimtask_counter(:,tc));
                                % Get nr of trials
                                nr_trials = sum(params.exp.nr_trials_per_block(1,tc)*nr_blocks);
                                % Get nr of catch trials
                                if nr_blocks > 0
                                    nr_catch_trials = ceil(nansum(total_catch_trials(:,tc))./sum(stimtask_counter(:,tc)));
                                else
                                    nr_catch_trials = 0;
                                end
                            else
                            ...
                            nr_catch_trials = ceil(total_catch_trials(sc,tc)./stimtask_counter(sc,tc));
                            % Get nr of blocks from the params struct
                            nr_blocks = stimtask_counter(sc,tc);
                            % Get nr of trials
                            nr_trials = (params.exp.nr_trials_per_block(sc,tc)*nr_blocks);
                        end
                        nr_catch_trials(isinf(nr_catch_trials))=0;
                        
                        if nr_trials > 0
                            if ismember(tc, [3,6]) % scc & ltm
                                ti = find(t(:,1)==99 & t(:,2)==tc);
                                if tc == 6 && sc == 5 % LTM-NS
                                    % number of unique conditions in a cycle
                                    nr_unique_conds = sum(params.exp.nr_unique_trials_per_crossing(sc,tc));
                                else % classic
                                    % number of unique conditions in a cycle
                            ...
                            % all trials we can allocate in condition master
                            trials_to_be_allocated   = find(allocated_trials(ti,:)==0);
                            % how many trials vs unique conditions do we have, can we expect to deal with a balanced distribution?
                            ...
                            while 1 % get trials for each stim-task crossing, separate for each session type
                                ...
                                while 1
                                    ...
                                    if reshuffle_me
                                        if tc ~= 6
                                            if mod(attempt,100)==0
                                                ...
                                                if length(trials_to_be_allocated) > nr_unique_conds
                                                    ...
                                                    if max(new_n) > length(trials_to_be_allocated)
                                                        new_n = length(trials_to_be_allocated);
                                                    end
                                                    idx = trials_to_be_allocated(1:new_n);
                                                end
                                        ...
                                        % we have enough trials to fill the
                                        % blocks
                                        elseif tc == 6
                                            ...
                                            while mod(sum(ns_stim), params.exp.nr_trials_per_block(sc,tc))>0 % if we sample too few NS trials, we have to change the idx.
                                                if length(trials_to_be_allocated) > nr_unique_conds % if we have extra trials
                                                    add_me    = params.exp.nr_trials_per_block(sc,tc) - mod(sum(ns_stim), params.exp.nr_trials_per_block(sc,tc));
                                                    if max(counter+[1:add_me]) < length(opt_trial)
                                                        idx0 = cat(1,trials_to_be_allocated([1:length(idx0)]), opt_trial(counter+[1:add_me]));
                                                        counter = counter+add_me;
                                                        ns_stim = condition_master.is_cued(ti(idx0)) == 3;
                                                    else
                                                        break;
                                                    end
                                                else
                                            ...
                                            while 1
                                                ns_stim = condition_master.is_cued(ti(idx0)) == 3;
                                                idx_ns      = idx0(ns_stim);
                                                idx_classic = idx0(~ns_stim);
                                                
                                                order0 = [shuffle_concat(idx_ns,1); shuffle_concat(idx_classic,1)];
                                                remainder = rem(length(order0),params.exp.nr_trials_per_block(sc,tc));
                                                if remainder > 0
                                                    remainder_idx = order0((length(order0)-remainder+1):end);
                                                else
                                                    remainder_idx = [];
                                                end
                                                order0 = order0(1:(end-remainder));
                                                order0 = reshape(order0,params.exp.nr_trials_per_block(sc,tc), []); % mix NS and classic blocks
                                        ...
                                        if tc == 6
                                            ns_stim = condition_master.is_cued(ti(idx)) == 3;
                                            idx_ns      = idx(ns_stim);
                                            idx_classic = idx(~ns_stim);
                                            
                                            order = [idx_ns; idx_classic]; % order NS vs Classic stim
                                        else
                                            order = idx;
                                        end
                                    end
                                    ...
                                    % Find trials in condition master that we can allocate
                                    ...
                                    if all(cued_conds==3) % neutral cues (fixation or NS trials)
                                        ...
                                        for bb = 1:nr_blocks
                                            bbi = dt(bb):(dt(bb)+params.exp.nr_trials_per_block(sc,tc)-1);
                                            % check if cueing left/right is balanced within a block.
                                            if all(cued_conds(bbi)==3)
                                                bb_ok(bb) = 1;
                                            ...
                                            % Check if we repeat the same
                                            % stimulus number, if we have more than two repeats per block then we reshuffle.
                                            % We allow for 2 repeats in a single-image block (which has 4 trials)
                                            % and 1 repeat within a double-image block (which has 8 trials)
                                            if (params.exp.nr_trials_per_block(sc,tc) == 4) && (length(stim_nr_left(~catch_idx(bbi))) - length(unique(stim_nr_left(~catch_idx(bbi)))) <= nr_unique_stim_repeat_double)
                                ...
                                if ismember(tc,[3,6]), curr_sc = 99; else, curr_sc = sc; end  % scc/ltm
                            ...
                            for xx = 1:nr_blocks
                                ...
                                if ~all(cue_tmp(xi)==3) && tc ~= 6
                                    assert(diff(histcounts(cue_tmp(xi),[1:3]))==0) % histcount(x,[1:3]) corresponds to bins = [0,1,2]
                                end
                            end
                            ...
                            if curr_tc ==3
                                unique_block_nr = sum(all_stimtask_counter(:,curr_tc)) + (1:nr_blocks);
                            else
