...
for sj = 1:p.exp.total_subjects
    ...
    for ses = 1:p.exp.n_sessions
        ...
        for rr = 1:p.exp.n_runs_per_session
            ...
            time_table = vcd_preallocateNaNTable(sz(1), sz(2), p.trials(1,:), []);
            ...
            tmp_durs = [p.exp.block.total_single_epoch_dur, p.exp.block.total_double_epoch_dur];
            ...
            if total_run_frames == 0
                ...
                time_table.event_dur(table_idx)      = p.exp.block.eye_gaze_fix0;
                time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx)-1;
                time_table.event_id(table_idx)       = p.exp.block.eye_gaze_fix_ID;
                ...
                sac_IDs = NaN(1,p.exp.block.nr_of_saccades);
                insert_fix_target = randi([2,p.exp.block.nr_of_saccades-1],1);
                sac_IDs(insert_fix_target) = p.exp.block.eye_gaze_sac_target_ID(1);
                sac_IDs(isnan(sac_IDs)) = shuffle_concat(p.exp.block.eye_gaze_sac_target_ID(2:end),1);
                
                for sac = 1:p.exp.block.nr_of_saccades
                    time_table.event_start(table_idx)    = total_run_frames;
                    time_table.event_dur(table_idx)      = p.exp.block.eye_gaze_sac_target;
                    time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx)-1;
                ...
                time_table.event_start(table_idx)    = total_run_frames;
                time_table.event_dur(table_idx)      = p.exp.block.eye_gaze_fix1;
                time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx)-1;
                time_table.event_id(table_idx)       = p.exp.block.eye_gaze_fix_ID;
                time_table.event_name(table_idx)     = {'et_fix'};
                time_table.run_nr(table_idx)         = rr;
                ...
                for ppl = 1:length(p.exp.block.eye_gaze_pupil)
                    time_table.event_start(table_idx)    = total_run_frames;
                    time_table.event_dur(table_idx)      = p.exp.block.eye_gaze_pupil(ppl);
                    time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx)-1;
                    time_table.event_id(table_idx)       = p.exp.block.eye_gaze_pupil_ID;
                ...
                [ibis, preblank_to_add, postblank_to_add] = ...
                    vcd_optimizeIBIs(p.exp.run.total_run_dur-p.exp.block.total_eyetracking_block_dur, ...
                        block_dur, p.exp.block.IBI, length(block_dur), sum([p.exp.run.post_blank_dur,p.exp.run.pre_blank_dur]));
                
                time_table.event_start(table_idx)    = total_run_frames;
                time_table.event_dur(table_idx)      = p.exp.run.pre_blank_dur + preblank_to_add;
                time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx)-1;
            ...
            while 1
                ...
                for id = 1:length(trial_IDs)
                    ...
                    switch trial_IDs(id)
                        ...
                        case p.exp.block.trial_start_ID % 94 trial start (dot thickening)
                        ...
                        case p.exp.block.spatial_cue_ID  % 95 spatial cue
                        ...
                        case p.exp.block.stim_epoch1_ID % 91 stim epoch 1 - left&right                           
                            tmp1 = t_trial(1,:);
                        ...
                        case p.exp.block.delay_ID % delay
                            time_table.event_dur(table_idx)      = p.exp.trial.delay_dur;
                            time_table.event_end(table_idx)      = time_table.event_start(table_idx) + time_table.event_dur(table_idx) - 1;
                            time_table.event_id(table_idx)       = trial_IDs(id);
                    ...
                    elseif curr_block == next_block_id  
                        ...
                        if curr_trial == 1 
                            ...
                            if time_table.trial_type(table_idx-2) == 1 % single stim epoch
                                max_nr_trials = p.exp.block.n_trials_single_epoch;
                                max_block_dur = p.exp.block.total_single_epoch_dur;
                                max_trial_dur = p.exp.trial.single_epoch_dur;
                            else % double stim epoch
                                max_nr_trials = p.exp.block.n_trials_double_epoch;
                                max_block_dur = p.exp.block.total_double_epoch_dur;
                                max_trial_dur = p.exp.trial.double_epoch_dur;
                            end
                            
                            itis = vcd_optimizeITIs(max_block_dur-p.exp.block.task_cue_dur, max_trial_dur, p.exp.trial.ITI, max_nr_trials);
                        end
            ...
            if run_finished || block_vec_idx > length(block_nrs) || (block_vec_idx+1)==length(block_nrs) % last trial of block and last block
                ...
                total_run_time2 = total_run_frames + p.exp.run.post_blank_dur + postblank_to_add;
                
                if total_run_time2 == p.exp.run.total_run_dur
                    post_blank_dur = p.exp.run.post_blank_dur + postblank_to_add + 1;
                elseif total_run_time2 < p.exp.run.total_run_dur
                    round_me_out = (ceil(total_run_time2/ tr_in_frames) - (total_run_time2/tr_in_frames)).*tr_in_frames;
                    post_blank_dur = p.exp.run.post_blank_dur + postblank_to_add + round_me_out;
                elseif total_run_time2 > p.exp.run.total_run_dur
                    post_blank_dur = p.exp.run.total_run_dur - total_run_frames + 1;
                end
                ...
                assert(nearZero(mod(total_run_frames,p.exp.run.total_run_dur)))
