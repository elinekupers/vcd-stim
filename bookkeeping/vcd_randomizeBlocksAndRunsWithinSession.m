            ...
            while restart_shuffle
                ...
                for run_idx = 1:length(run_order)
                    ...
                    for bb_idx = 1:length(curr_block_start)
                        ...
                        while trial_order_not_ok
                            ...
                            if attempt > 2*50000
                                if params.is_demo
                                    ...
                                    warning('\n[%s]: Can''t reach a trial order without repeats! Will rerun block randomization code',mfilename);
                                    restart_shuffle = true;
                                    break;
                                end
                            end
                        ...
                        % break out of block for loop
                        if restart_shuffle
                            break;
                        end
                    end
                    % break out of run for loop
                    if restart_shuffle
                        break;
                    end
                end
