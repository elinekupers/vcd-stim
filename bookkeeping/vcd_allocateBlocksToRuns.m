...
% keep track of all the sessions, blocks, trials allocated across sessions
    ...
    for st = 1:size(all_sessions,4) % session types
                ...
                if  ses >= task_start
                    ...
                    if (n_blocks > 0)
                        ...
                        for ii = 1:length(curr_blocks)
                            ...
                            condition_master.crossing_nr(idx)       = find(strcmp(blck_name,params.exp.crossingnames));
        ...
        for sj = 1:params.exp.total_subjects
            ...
            for run_idx = 1:size(curr_runs_shuffled,1)
                ...
                global_block_counter = global_block_counter + length(block_in_this_run);
                ...
                for iii = 1:length(subj_ses_block_shuffle)
                    ...
                    condition_master.global_trial_nr(idx)   = global_trial_counter;
