...
for bb = 1:size(cellblock,2)
    ...
    for tt = 1:length(im_nr)
        ...
        if im_nr{tt}==0 % blockstart
            ...
            seq_duration = cat(1,seq_duration, tmp_timing.event_dur(tt));
        ...
        % 93: response ID // 94: trial_start_ID // 95: spatial_cue_ID // 96:delay_ID = 96    
            ...
            seq_duration         = cat(1,seq_duration, params.exp.trial.start_cue_dur);
            ...
            seq_duration         = cat(1,seq_duration, params.exp.trial.spatial_cue_dur);
            cumultime            = cumultime + params.exp.trial.spatial_cue_dur;
            
            stim_im              = im_nr(tt);
            stim_run_time        = cumultime; 
            seq_duration         = cat(1,seq_duration, params.exp.trial.stim_array_dur);
            ...
            if cellblock{5,bb} == 2
                ...
                seq_duration        = cat(1,seq_duration, params.exp.trial.delay_dur);
                ...
                seq_duration        = cat(1,seq_duration, params.exp.trial.response_win_dur);
...
seq_duration    = cat(1, seq_duration(2:end),0);
...
timing.seq_duration    = seq_duration;
...
while fix_timing < (seq_timing(end)-fudge)
    fix_dur = [fix_dur, fixsoafun()];
    fix_timing = cumsum(fix_dur);
    fix_seq = [fix_seq, randi(length(params.stim.fix.dotlum),1)];
end

for tt = 1:length(fix_timing)
    
    event_type = seq_stim(tt,1);
    event_time = fix_timing(tt);
    
    if any(event_type==[0,98,99]) % pre/post-block, iti, ibi
        fix_seq_im{tt} = fix_im(:,:,:,fix_seq(tt), 1); % thin rim
    elseif any(event_type==[93,94,96,97]) % trial start
        fix_seq_im{tt} = fix_im(:,:,:,fix_seq(tt), 2); % thick rim
    elseif event_type==95 && trig_spatial_cue(tt)==1
        fix_seq_im{tt} = fix_im(:,:,:,fix_seq(tt), 3); % thick rim left
    elseif event_type==95 && trig_spatial_cue(tt)==2
        fix_seq_im{tt} = fix_im(:,:,:,fix_seq(tt), 4); % thick rim right
    end
    fix_seq_im_mask{tt} = fix_im_mask(:,:,:,fix_seq(tt), 1); % only alpha mask fits all

end

timing.fix_dur         = fix_dur;
timing.fix_timing      = fix_timing;
timing.fix_seq_im      = fix_seq_im;
timing.fix_seq         = fix_seq;
timing.fix_seq_im_mask = fix_seq_im_mask;
...
gauss_win_t_sec = params.stim.cd.t_gauss.*params.stim.framedur_s;
...
if ~isempty(cdID)
    for ii = cdID
        ...
        blockOnset = find(seq_block(:,1)==subj_run.block(ii).ID);
        blockOnset = blockOnset(1);
        foo = (trig_block(blockOnset:end,1)~=seq_block(blockOnset,1));
        foo = find(foo); blockOffset = blockOnset+foo(1)-1; clear foo
        
        stimOnset      = (seq_stim(blockOnset:blockOffset,1)>0 & seq_stim(blockOnset:blockOffset,1)<90);
        ...
        for ss = 1:length(stim0)
            trig_stim_time0 = seq_timing(stimOnset_all(ss));
            ...
            for nn = 1:size(subj_run.block(ii).trial,2)
                tmp_dur = cdsoafun();
                cd_duration = [cd_duration, gauss_win_t_sec(end)];
                onset_decrement_s = seq_timing(stimOnset_all(ss)) + tmp_dur;
                
                t_idx = onset_decrement_s + gauss_win_t_sec;
                ...
                for tt = 1:length(t_idx)
                    ...
                    if strcmp(subj_run.block(ii).name,'cd-ns') % range from [0 254]
                       ...
                       seq_exp_im_w_cd{t_idx(tt)}{nn} = tmp_im_c;
                        
                    elseif strcmp(subj_run.block(ii).name,'cd-cobj')
                        tmp_im = double(timing.seq_exp_im{tt}{nn});
                        sz0 = size(tmp_im);
                        ...
                        seq_exp_im_w_cd{t_idx(tt)}{nn} = reshape(tmp_im_c,sz0);
                    else % RDK or DOT or GABOR
                        tmp_im = double(timing.seq_exp_im{tt}{nn});
                        sz0 = size(timing.seq_exp_im{tt}{nn});
...
timing.cd_timing       = cd_timing;
timing.cd_duration     = cd_duration;
timing.seq_exp_im_w_cd = seq_exp_im_w_cd;
