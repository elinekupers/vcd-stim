function all_trials = vcd_makeTrials(p,load_params,store_params)
...
if load_params
    ...
    d = dir(fullfile(vcd_rootPath,'workspaces','info',sprintf('trials_%s*.mat',dispname)));
    if ~isempty(d)
        if length(d) > 1
...
%                                  clear conds_shuffle0 conds_shuffle1 conds_shuffle2
                    ...
                    for mm = 1:size(miniblock_trials,2)
                        for local_trial_nr = 1:size(miniblock_trials,1)
                            for stimloc = [1,2]
                                ...
                                trial(local_trial_nr,stimloc).paired_stim        = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),10);
                                trial(local_trial_nr,stimloc).lure               = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),11);
                                trial(local_trial_nr,stimloc).repeat_nr          = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),12);
            ...
            case 'rdk'
                ...
                for curr_task = 1:length(task_crossings)
                    ...
                    trial = ...
                        ...
                        'paired_stim',[],'lure',[],'ref_delta',[],'covert_att_loc_cue', []);
                    ...
                    for mm = 1:size(miniblock_trials,2)
                        for local_trial_nr = 1:size(miniblock_trials,1)
                            for stimloc = [1,2]
                                ...
                                trial(local_trial_nr,stimloc).paired_stim        = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),9);
                                trial(local_trial_nr,stimloc).lure               = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),10);
            ...
            case 'dot'
                ...
                for curr_task = 1:length(task_crossings)
                    ...
                    trial = ...
                        ...
                        'paired_stim',[],'lure',[],'trial_nr',[],'stim_loc',[],'ref_delta',[],'covert_att_loc_cue', []);
                    ...
                    for rep = 1:n_repeats
                        ...
                        % Keep track of repeat
                        rep_vec = rep.*ones(size(conds_master_single_rep3,1),1);
                        conds_master_single_rep3 = [conds_master_single_rep3, angle2, pair_vec, lure_vec, rep_vec];
                    ...
                    for mm = 1:size(miniblock_trials,2)
                        for local_trial_nr = 1:size(miniblock_trials,1)
                            for stimloc = [1,2]
                                ...
                                trial(local_trial_nr,stimloc).paired_stim        = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),8);
                                trial(local_trial_nr,stimloc).lure              = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),9);
                    ...
                    for mm = 1:size(miniblock_trials,2)
                        for local_trial_nr = 1:size(miniblock_trials,1)
                            for stimloc = [1,2]
                                ...
                                unique_obj_names = unique(p.stim.cobj.basic_cat{trial(local_trial_nr,stimloc).super_cat},'stable');
                                trial(local_trial_nr,stimloc).basic_cat_name    = unique_obj_names(trial(local_trial_nr,stimloc).basic_cat);
                                ...
                                trial(local_trial_nr,stimloc).paired_stim       = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),11);
                                trial(local_trial_nr,stimloc).lure              = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),12);
                                trial(local_trial_nr,stimloc).repeat_nr         = cond_master(miniblock_trials(local_trial_nr,mm)+(stimloc-1),13);
            ...
            case 'ns'
                ...
                for curr_task = 1:length(task_crossings)
                    ...
                    trial = ...
                        ...
                        'basic_cat',[],'basic_cat_name',[],...
                        'sub_cat',[],'sub_cat_name',[],...
                        'change_blindness',[],'change_blindness_name',[],...
                        'lure',[],'lure_name',[],'covert_att_loc_cue',[]);
                    ...
                    for rep = 1:n_repeats
                        ...
                        % Keep track of repeat
                        rep_vec = rep.*ones(size(conds_master_single_rep3,1),1);
                        conds_master_single_rep3 = [conds_master_single_rep3, change_blindness_vec, pair_vec, lure_vec, rep_vec]; % change blindness: 1 = easy, 2 = hard
                    ...
                    for mm = 1:size(miniblock_trials,2)
                        for local_trial_nr = 1:size(miniblock_trials,1)
                            ...
                            if ~isnan(trial(1,local_trial_nr).change_blindness)
                                ...
                                trial(1,local_trial_nr).change_blindness_name   = [];
                            end
                            if ~isnan(trial(1,local_trial_nr).lure)
                                trial(1,local_trial_nr).lure_name   = p.stim.ns.lure_im{cond_master(miniblock_trials(local_trial_nr,mm),10)};
                            else
                                trial(1,local_trial_nr).lure_name   = [];
                            end
    ...
    if store_params
        ...
        save(fullfile(saveDir,sprintf('trials_%s_%s.mat',p.disp.name,datestr(now,30))),'all_trials','all_unique_im','all_cond_master','priority_stim_manip')
    end
end
